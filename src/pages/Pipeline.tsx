...
export function Pipeline() {
  ...
  const reload = useCallback(() => v1.sources().then((data) => setSources(data.sources)), []);
  ...
  return <div className="shell page">
    ...
    <section className="register">
      <div className="register-head"><p className="meta">02 / Source registry</p><span className="meta">URLs remain encrypted at rest</span></div>
      <div className="source-list">
        {sources.map((source) => <Link key={source.id} to={`/pipeline/sources/${source.id}`} className="source-row"><span><span className="mono">SRC / {source.id.slice(-8).toUpperCase()}</span><strong>{source.title || source.domain}</strong></span><span className="meta">{source.domain}</span><SourceStatus source={source} /><span className="arrow">→</span></Link>)}
...
export function PipelineSource() {
  ...
  useEffect(() => { reload().catch((error) => setNotice(error instanceof Error ? error.message : "Source unavailable.")); }, [reload]);
  ...
  return <div className="shell page">
    <Link className="btn-ghost" to="/pipeline">← Durable pipeline</Link>
    ...
    <section className="source-section"><p className="meta">03 / Approval and immutable ingestion</p><p className="body-sm" style={{ marginTop: 12 }}>Approval records the operator’s licensing decision. Scrape runs fetch the actual source in a worker, preserve raw bytes, then derive a separate normalized version.</p><div className="source-actions" style={{ marginTop: 18 }}>{source.status === "candidate" || source.status === "approved" ? <button className="btn btn-secondary" disabled={Boolean(busy)} onClick={() => act("Approval", async () => { await v1.approve(source.id); })}>{busy === "Approval" ? "Approving…" : "Approve source"}</button> : null}<button className="btn btn-primary" disabled={Boolean(busy) || source.status !== "active"} onClick={() => act("Scrape", async () => { await waitForJob((await v1.scrape(source.id)).job); })}>{busy === "Scrape" ? "Scraping…" : "Fetch and normalize now"}</button></div></section>
...
export function PipelineAnalysis() {
  ...
  const [busy, setBusy] = useState(false);
  const reload = useCallback(async () => { const next = await v1.analysis(id); setData(next); setNotice(""); }, [id]);
  useEffect(() => { reload().catch((error) => setNotice(error instanceof Error ? error.message : "Analysis unavailable.")); }, [reload]);
  useEffect(() => {
  ...
  const createPackage = async () => { try { setBusy(true); const created = await v1.createPackage(id, title, seats); setPackageId(created.package.id); setNotice("Package sealed. It remains private until you explicitly publish it to Devnet."); } catch (error) { setNotice(error instanceof Error ? error.message : "Package sealing failed."); } finally { setBusy(false); } };
  const publish = async () => { try { setBusy(true); const job = await v1.publishPackage(packageId); setNotice(`Devnet publication queued as ${job.job.id}. The isolated signer will only mark it committed after finalization.`); } catch (error) { setNotice(error instanceof Error ? error.message : "Publication request failed."); } finally { setBusy(false); } };
  ...
  return <div className="shell page">
    ...
    <section className="source-section"><p className="meta">Leakage and bias controls</p><div className="source-list" style={{ marginTop: 14 }}>{data.leakage_checks.map((check) => <div className="source-row" key={`${value(check, "check_type")}-${value(check, "signal_candidate_id")}`}><span><span className="mono">{value(check, "check_type")}</span><strong>{value(check, "status")}</strong></span><span className="meta">penalty {value(check, "score_penalty")}</span><span className="meta">{value(check, "explanation")}</span></div>)}</div></section>
    <section className="source-section"><p className="meta">Candidate signals and out-of-sample validation</p><div className="source-list" style={{ marginTop: 14 }}>{data.signal_candidates.slice(0, 20).map((signal) => <div className="source-row" key={value(signal, "id")}><span><span className="mono">{value(signal, "transformation")}</span><strong>{value(signal, "source_column")}</strong></span><span className="meta">lag {value(signal, "lag")} · horizon {value(signal, "horizon")}</span><span className="meta">{data.validations.filter((item) => item.signal_candidate_id === signal.id && item.split === "test").map((item) => `test IC ${Number(item.information_coefficient ?? 0).toFixed(3)}, Sharpe ${Number(item.sharpe_like ?? 0).toFixed(2)}`).join(" · ") || "validation pending"}</span></div>)}</div></section>
    <section className="source-section"><p className="meta">06 / Private package and on-chain provenance</p><p className="body-sm" style={{ marginTop: 12 }}>Package sealing writes immutable policy and private metadata. Publication is a separate explicit Devnet action; it creates the DatasetCommitment and limited-seat sale through the program.</p><div className="target-controls" style={{ marginTop: 16 }}><input className="source-input" value={title} onChange={(event) => setTitle(event.target.value)} aria-label="Package title" /><input className="source-input" type="number" min="1" max="10000" value={seats} onChange={(event) => setSeats(Number(event.target.value))} aria-label="Maximum seats" /><button className="btn btn-secondary" disabled={!complete || busy || Boolean(packageId)} onClick={createPackage}>Seal package</button>{packageId && <button className="btn btn-primary" disabled={busy} onClick={publish}>Publish to Devnet</button>}</div>{!complete && <p className="meta gate-error" style={{ marginTop: 12 }}>Package creation stays disabled until the analysis is completed without blocking leakage.</p>}</section>
    {notice && <p className="meta gate-error" style={{ marginTop: 20 }}>{notice}</p>}
  </div>;
}
