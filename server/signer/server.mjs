...
import { publishPackageOnChain } from "./publisher.mjs";
...
const server = createServer(async (request, response) => {
  ...
  if (request.method !== "POST" || request.url !== "/publish" || !allowed(request)) { response.writeHead(404, { "content-type": "application/json" }); response.end('{"error":"not_found"}'); return; }
  try { const result = await publishPackageOnChain({ signer, rpcUrl: config.values.SOLANA_RPC_URL, programId: config.values.SOLANA_PROGRAM_ID, input: await body(request) }); response.writeHead(200, { "content-type": "application/json", "cache-control": "no-store" }); response.end(JSON.stringify(result)); }
